...
### 一、为什么 PCIe 不再依赖传统 interrupt pin

传统 interrupt pin 只能表达“有事件需要处理”，但不能自然携带多个 queue、多个 vector 或多个 software handler 的身份。

MSI 的概念变化是把 interrupt 从一根共享的电平信号，变成一笔包含 address 与 data 的 Memory Write。这样 Root Complex 可以像处理普通 transaction 一样路由 interrupt，software 也可以通过 message data 区分来源。

MSI-X 进一步把一个 Function 的多个 interrupt source 拆成独立 vector。每个 vector 都可以配置不同 message destination，因此多 queue device、virtualization 和多 CPU handler 都能获得更清晰的 interrupt isolation。

### 二、MSI 为什么是一笔 Memory Write
...
### 五、Function mask 与 vector mask 为什么要分开

Function-level enable／mask 控制整个 MSI-X capability 是否可用。vector mask 则只控制某一个具体 vector。前者适合整体停用 interrupt，后者适合 software 临时屏蔽某个 queue 或某类 event。

这两个层次不能混淆。Function-level disable 时，不应继续产生 interrupt write。单个 vector 被 mask 时，其他 vector 仍可以正常发 message，而被 mask vector 的 event 应根据 PBA／设计 policy 保留 pending 信息。

### 六、Function isolation 与 FLR
...
### 七、Code demo：MSI-X delivery state machine

![MSI-X interrupt delivery checker 伪代码](pcie_msix-code-demo.png)

这段伪代码的重点不是具体实现，而是验证顺序：先确认 Function 是否 enable，再检查 vector mask，再决定发 message write 或记录 pending。unmask 后还要检查 pending event 是否按设计规则被重新处理。
