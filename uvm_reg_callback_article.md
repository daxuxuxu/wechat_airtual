...
> 2. 📖 **本文**：Callback 机制——在读写流程中插入自定义逻辑
> 3. 📖 predict(DIRECT)：Lock bit 场景下的 scoreboard 误报修复
>
> **前置知识**：阅读本文前，建议先了解 UVM reg model 的 desired/mirrored 机制和 auto-predict 的工作原理（见第一篇）。核心概念速查：
> - `mirrored`：reg model 对硬件当前状态的认知镜像，scoreboard 预测基于此
> - `auto-predict`：`write()` 完成后自动更新 mirrored 的机制

---

**读完本文，你将理解**：

- Callback 解决了什么问题，为什么静态寄存器定义不够用
- `post_predict()` hook 的参数语义和修改方式
- 写保护 Callback 如何与 auto-predict 协作
...
同一目标可以注册多个 Callback，执行时按注册顺序依次调用，形成调用链：

1. `write()` 完成，auto-predict 开始执行
2. **Callback_1.post_predict()** 被调用（先注册先执行），可修改 `value` 参数
3. **Callback_2.post_predict()** 被调用，收到 Callback_1 修改后的 `value`
4. 最终 `value` 写入 `mirrored`

每个 Callback 可以通过修改 `value`（inout 参数）来影响后续 Callback 和最终写入 mirrored 的值。
...
> **注意**：下面的 `lock_keep_callback` 是**用户自定义类**，名称和内部逻辑均需根据实际硬件行为编写，不是 UVM 内置组件，直接复制不能使用。
...
1. 如果对同一个字段注册了两个 Callback，第一个将 value 从 0 改成 1，第二个检查"如果 previous==1 则保持 value==1"。当字段 mirrored 原为 0 时，执行一次 `write(0)` 后，mirrored 最终是多少？

2. `post_write()` 和 `post_predict()` 都在 write 完成后触发，它们有什么区别？什么时候用 `post_write()`，什么时候用 `post_predict()`？

3. 硬件复位后，你用 `predict(DIRECT, 0)` 将某字段的 mirrored 清零。但如果该字段注册了一个 lock_keep Callback（previous==1 时阻止清零），这次 DIRECT predict 会触发 Callback 吗？结果是什么？

---

## 参考答案

**1.** mirrored 最终为 **1**。执行过程：write(0) 触发 auto-predict，调用 `predict(UVM_PREDICT_WRITE, 0)`。Callback_1 先执行，将 value 从 0 改为 1。Callback_2 收到 value=1，检查 previous==0（非 1），不做修改，value 保持 1。最终 mirrored = 1。注意：previous 是 predict 前的 mirrored（=0），不是 Callback_1 修改后的值，两个 Callback 看到的 previous 相同。

**2.** `post_write()` 是 task（可以阻塞），在总线事务发送后、auto-predict 执行前触发，适合触发 side effect（如驱动另一个序列、等待某个条件）。`post_predict()` 是 function（不能阻塞），在 auto-predict 执行后触发，适合修正 mirrored 的值（如 lock_keep、截断对齐等）。需要修改 mirrored 结果 → 用 `post_predict()`；需要触发外部动作 → 用 `post_write()`。

**3.** `predict(DIRECT)` **不触发**任何 Callback，包括 lock_keep。`UVM_PREDICT_DIRECT` 的设计目标正是绕过所有 Callback，无条件强制设置 mirrored。结果：mirrored 被清零为 0，lock_keep Callback 不介入。这也是硬件复位后同步 mirrored 必须用 DIRECT 而不是普通 predict 的原因。
