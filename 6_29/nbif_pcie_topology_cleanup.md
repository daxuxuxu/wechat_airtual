...
### RC+EP 拓扑（集成 GPU / APU 模式）

没有 Switch 层级，RC 直连 EP。特点：

- 不存在 US 设备（`us_dev == null`）
- 不存在 DS Port 设备
- EP Functions 直接挂在 RC 下
- 事务路由简单，RC 直接寻址 EP 的 BAR

### US+DS+EP 拓扑（独立 GPU / Switch 模式）

有完整的 Switch 层级：

- **US Func（SWUS）**：Switch 的上游口，负责把 EP 到 RC 方向的事务路由回去
- **DS Func（dwn_pf）**：Switch 的下游口，负责把 RC 到 EP 方向的事务路由下去
- **EP Functions**：挂在 DS 口下，实际工作的设备功能

```
验证环境中的设备对象树

RC+EP 模式：                        US+DS+EP 模式：
nbif_info                           nbif_info
  us_dev = null                       us_dev (存在)
  dev_array[0]                        dev_array[0]
    ep_dev                              ds_dev (存在)
      pfs[0,1,...]                      ep_dev
                                          pfs[0,1,...]
```

判断当前拓扑最直接的方式：检查 `us_dev` 是否为 null。
