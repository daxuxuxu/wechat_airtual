...
`AxADDR` 是 burst 的 start address。`AxLEN` 编码 burst length 减一，因此实际 beat 数是 `AxLEN + 1`。`AxSIZE` 编码每拍 transfer 的有效 byte 数。`AxBURST` 定义相邻 beat 的 address 规则。

一个 **beat** 是 burst 中的一次 data transfer。`WLAST` 标记最后一个 write data beat，`RLAST` 标记最后一个 read data beat。

Burst 的目的不是只减少 address 数量，而是用一次 address transaction 描述多个有顺序关系的 data beat，从而提高连续传输效率。
