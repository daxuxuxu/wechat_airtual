...
`AxBURST` 定义 burst 中 address 的推进规则。FIXED 每拍保持同一 address。INCR 每拍递增 bytes per beat。WRAP 则在合法 boundary 范围内递增并回绕。

![AXI Burst Type](axi-burst-types.png)

DV 不能只检查第一拍地址。FIXED 要确认所有 beat 地址保持一致。INCR 要确认每拍 increment 与 `AxSIZE` 一致。WRAP 要确认 wrap boundary、burst length 和 address progression 同时满足协议约束。
