...
LTSSM 不是只按一条直线走到底。link 正常 bring-up 时会从 Detect 经过 Polling、Configuration 进入 L0；进入 L0 后又可能因为低功耗、retrain 或 error recovery 走向其他状态。

![LTSSM 常见状态转移](ltssm-state-transition.png)

从 L0 进入 L0s 或 L1，通常是为了降低 link 功耗。新的 traffic 或 wake 条件到来后，link 会返回 L0。

从 L0 进入 Recovery，通常表示 link 需要重新训练，例如 retrain、speed change、lane 问题或 recovery。Recovery 成功后返回 L0；若 reset 或 disable 发生，则 link 会重新走检测和训练路径。
