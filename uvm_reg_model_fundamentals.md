...
```
reg model 发起 write(addr=0x1000, data=0xA5)
              ↓
         reg2bus() 被调用
              ↓
   输入：uvm_reg_bus_op {kind=WRITE, addr=0x1000, data=0xA5}
              ↓
   判断：这是 PCIe Config Space 还是 SMN MEM 空间？
              ↓
   构造：协议特定的 bus transaction
         (如：CFG_WR_TYPE0, addr=0x1000, data=0xA5,
              byte_enable=0xF, tag=0x01, ...)
              ↓
   返回：bus sequencer 可以发送的 sequence_item
```
...
```
monitor 捕获到 bus transaction（写响应 / 读完成帧）
              ↓
         bus2reg() 被调用
              ↓
   输入：协议特定的 bus_item
   ...
   解析：从帧格式中提取
         - addr:   物理地址
         - data:   读操作的返回数据
         - kind:   这是 READ 完成还是 WRITE 完成
         - status: 操作成功还是出错（UR / CA / timeout）
              ↓
   输出：填充 uvm_reg_bus_op rw（ref 参数）
              ↓
reg model 收到结果：触发 auto-predict，更新 mirrored
```
...
reg.write(value)
    │
    ▼
reg2bus(rw)  ──────→  bus_sequencer → driver → DUT
                                                 │
                                                 ▼
scoreboard ←── predictor ←── reg model ←── bus2reg(rsp_item)
                (基于 mirrored)           (monitor 捕获)
```
...
Driver ──→ Bus ──→ DUT                             │
               │                                  │
               ▼                                  │
          Monitor ──→ Predictor ←──── mirrored ──┘
                          │
                          ▼ 生成预期响应
                      Scoreboard
                          │
                          ├── 与 actual response 比较
                          └── 报告 PASS / FAIL
```
